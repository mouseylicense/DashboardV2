...
try:
    with open("./data/problems.json","r",encoding="utf-8") as my_file:
        classes = json.load(my_file)
except: 
    classes = {}

try:
    with open("./data/users.json","r",encoding="utf-8") as my_file:
...
def save(classes):
    with open("./data/problems.json","w",encoding="utf-8") as my_file:
        json.dump(classes,my_file,ensure_ascii=False)

def save_users(users):
    with open("./data/users.json","w",encoding="utf-8") as my_file:
...
def video():
    ...
    if request.method == "POST":
        ...
        if link:
            ...
            yt.download(filename=yt.title[:8]+"....mp4",output_path="./static/videos")
            video_path = yt.title[:8]+"....mp4"
        else:
            print("not")
            video_path = request.form.get("video")
        
    return render_template("video.html",videos= [f for f in listdir("static/videos/") if isfile(join("static/videos/", f))])
